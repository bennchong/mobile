...
test("encrypt, then decrypt", () => {
  cryptoHelper
    .encryptString(JSON.stringify(message))
    .then(result => {
      return cryptoHelper.decryptString(result.encryptedString, result.key);
    })
    .then(data => {
      const jsonObject = JSON.parse(data.data);
      console.log(jsonObject);
      expect(jsonObject).toBe(message);
    });
});

// const res = cryptoHelper
//   .encryptString(JSON.stringify(message))
//   .then(result => {
//     return cryptoHelper.decryptString(result.encryptedString, result.key);
//   })
